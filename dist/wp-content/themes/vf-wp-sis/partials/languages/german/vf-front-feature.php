    ...
    <div class="vf-card-container__inner">
        ...
        <div class="vf-section-header">
            <h2 class="vf-section-header__heading vf-section-header__heading--is-link">
                <a href="<?php echo esc_url($currentIssueLink); ?>">Aus der aktuellen Ausgabe</a>
        ...
        if ($query->have_posts()) {
            while ($query->have_posts()) {
                ...
                $german_id = apply_filters('wpml_object_id', get_the_ID(), 'sis-article', false, 'de'); // Get the German ID
                
                if ($original_id && !in_array($original_id, $original_posts)) {
                    $original_posts[] = $original_id; // Only add if it's not already in the array
                }
                if ($german_id && !in_array($german_id, $german_posts)) {
                    $german_posts[] = $german_id; // Only add if it's not already in the array
                }
            }
        }
        wp_reset_postdata();
        ...
        $german_posts = array_unique($german_posts);

        // Step 2: Determine how many posts to show and in what order
        $posts_to_show = [];

        if (count($german_posts) == 1) {
            // If there is 1 German post, show it first, then fill with 2 English posts
            shuffle($german_posts); // Shuffle to display randomly if there are more than one
            $posts_to_show[] = array_shift($german_posts); // First German post
        ...
        } elseif (count($german_posts) == 2) {
            // If there are 2 German posts, display them first, then 1 random English post
            shuffle($german_posts); // Shuffle to display randomly
            $posts_to_show[] = array_shift($german_posts); // First German post
            $posts_to_show[] = array_shift($german_posts); // Second German post
            
            // Add 1 random English post
            shuffle($original_posts); // Shuffle the remaining English posts
            $posts_to_show = array_merge($posts_to_show, array_slice($original_posts, 0, 3));

        } elseif (count($german_posts) >= 3) {
            // If there are 3 or more German posts, display 3 random German posts
            shuffle($german_posts); // Shuffle to display randomly
            $posts_to_show = array_merge($posts_to_show, array_slice($german_posts, 0, 3));
        
        } else {
            // If there are no German posts, display 3 random English posts
