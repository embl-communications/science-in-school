    ...
    <div class="vf-card-container__inner">
        ...
        <div class="vf-section-header">
            <h2 class="vf-section-header__heading vf-section-header__heading--is-link">
                <a href="<?php echo esc_url($currentIssueLink); ?>">Extrait du numéro actuel</a>
        ...
        if ($query->have_posts()) {
            while ($query->have_posts()) {
                ...
                $french_id = apply_filters('wpml_object_id', get_the_ID(), 'sis-article', false, 'fr'); // Get the french ID
                
                if ($original_id && !in_array($original_id, $original_posts)) {
                    $original_posts[] = $original_id; // Only add if it's not already in the array
                }
                if ($french_id && !in_array($french_id, $french_posts)) {
                    $french_posts[] = $french_id; // Only add if it's not already in the array
                }
            }
        }
        wp_reset_postdata();
        ...
        $french_posts = array_unique($french_posts);

        // Step 2: Determine how many posts to show and in what order
        $posts_to_show = [];

        if (count($french_posts) == 1) {
            // If there is 1 french post, show it first, then fill with 2 English posts
            shuffle($french_posts); // Shuffle to display randomly if there are more than one
            $posts_to_show[] = array_shift($french_posts); // First french post
        ...
        } elseif (count($french_posts) == 2) {
            // If there are 2 french posts, display them first, then 1 random English post
            shuffle($french_posts); // Shuffle to display randomly
            $posts_to_show[] = array_shift($french_posts); // First french post
            $posts_to_show[] = array_shift($french_posts); // Second french post
            
            // Add 1 random English post
            shuffle($original_posts); // Shuffle the remaining English posts
            $posts_to_show = array_merge($posts_to_show, array_slice($original_posts, 0, 3));

        } elseif (count($french_posts) >= 3) {
            // If there are 3 or more french posts, display 3 random french posts
            shuffle($french_posts); // Shuffle to display randomly
            $posts_to_show = array_merge($posts_to_show, array_slice($french_posts, 0, 3));
        
        } else {
            // If there are no french posts, display 3 random English posts
