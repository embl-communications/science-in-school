...
class FG_Drupal_to_WordPress_Premium {
	...
	private function load_dependencies() {
		...
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/class-fg-drupal-to-wp-toolset.php';
		...
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/class-fg-drupal-to-wp-link-field.php';
	...
	private function define_admin_hooks() {
		...
		$this->loader->add_action( 'fgd2wp_post_import', $plugin_users, 'import_users', 9 ); // Must import the users before the comments and the custom posts
		...
		$this->loader->add_action( 'fgd2wp_post_test_database_connection', $plugin_custom_content, 'check_types_activation' );
		 ...
		 * Link field
		 */
		$plugin_link_field = new FG_Drupal_to_WordPress_Link_Field( $plugin_admin );
		$this->loader->add_filter( 'fgd2wp_post_get_custom_field', $plugin_link_field, 'get_link_custom_fields', 10, 2 );
		...
		$this->loader->add_filter( 'fgd2wp_get_nodes_count', $plugin_media_entity, 'get_nodes_count', 10, 3 );
		 ...
		 * Toolset
		 */
		$plugin_toolset = new FG_Drupal_to_WordPress_Toolset( $plugin_admin );
		$this->loader->add_action( 'fgd2wp_post_empty_database', $plugin_toolset, 'delete_toolset_data' );
